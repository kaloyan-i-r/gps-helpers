...
- **Interval**: 2 seconds between points
- **Simplification**: 8 meters tolerance
- **Max Speed**: 45 m/s (162 km/h)
- **Min Distance**: 2 meters between points
- **Precision**: 5 decimal places (~1.1m accuracy)
- **Elevation**: Dropped
- **Extensions**: Stripped
- **Metadata**: Removed
...
- **Interval**: 1.5 seconds between points
- **Simplification**: 5 meters tolerance
...
- **Interval**: 1 second between points
- **Simplification**: 2 meters tolerance
...
| `--precision DECIMALS` | int | Lat/lon decimal places | 5 ≈ 1.1m, 6 ≈ 0.11m, 7 ≈ 0.01m. Lower = smaller file size but less precision. |
| `--no-resample` | flag | Skip resampling, keep original timing | Preserves original GPS timestamps. Only cleans and simplifies. |
| `--add-timestamps` | flag | Generate timestamps for files without them | Processes GPX files that only have waypoints/routes without timestamps. Creates synthetic timestamps based on distance and profile speed. |
| `--zip` | flag | Create fixed.zip archive | Creates a ZIP file containing all processed GPX files from the fixed/ folder (batch mode only). |
